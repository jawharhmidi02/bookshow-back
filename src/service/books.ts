...
export async function getBooksOrderTime() {
  return db.select().from(books).orderBy(asc(books.created_at));
}
...
export async function updateBookById(
  id: SelectBooks['id_book'],
  data: Partial<Omit<SelectBooks, 'id_Book'>>,
) {
  const response = await db
    .update(books)
    .set(data)
    .where(eq(books.id_book, id))
    .returning();
  return response;
}
...
export async function updateBookCoverById(
  id: SelectBooks['id_book'],
  file: any,
) {
  cloudinary.config({
...
export async function updateBookPdfUrlById(
  id: SelectBooks['id_book'],
  file: any,
) {
  cloudinary.config({
