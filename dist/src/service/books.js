...
async function getBooksOrderTime() {
    return connect_1.db
        .select()
        .from(books_1.books)
        .orderBy((0, drizzle_orm_1.asc)(books_1.books.created_at));
}
...
async function updateBookById(id, data) {
    const response = await connect_1.db.update(books_1.books).set(data).where((0, drizzle_orm_1.eq)(books_1.books.id_book, id)).returning();
    return response;
}
async function deleteBook(id) {
    const respone = await connect_1.db.delete(books_1.books).where((0, drizzle_orm_1.eq)(books_1.books.id_book, id)).returning();
    return respone;
}
async function updateBookCoverById(id, file) {
    ...
    const dbResponse = await connect_1.db.update(books_1.books).set({ cover_url: response.secure_url }).where((0, drizzle_orm_1.eq)(books_1.books.id_book, id)).returning();
    return dbResponse;
}
async function updateBookPdfUrlById(id, file) {
    ...
    const dbResponse = await connect_1.db.update(books_1.books).set({ pdf_url: response.secure_url }).where((0, drizzle_orm_1.eq)(books_1.books.id_book, id)).returning();
    return dbResponse;
}
